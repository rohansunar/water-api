...
export class VendorJwtStrategy extends PassportStrategy(Strategy, 'vendor-jwt') {
  ...
  async validate(payload: VendorJwtPayload): Promise<any> {
    this.customLogger.log(`[DEBUG] VendorJwtStrategy.validate invoked for vendor ID: ${payload.sub}, email: ${payload.email}`);
    this.customLogger.debug(`VendorJwtStrategy.validate called with payload: ${JSON.stringify(payload)}`);
    try {
      const vendor = await this.vendorAuthService.validateVendor(payload.sub);
      this.customLogger.debug(`Vendor validation successful for vendor ID: ${payload.sub}`);
      return {
      ...
      this.customLogger.logSecurityEvent('vendor_jwt_validation_failure', {
