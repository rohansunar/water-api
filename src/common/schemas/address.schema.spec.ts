...
describe('Address Schema', () => {
  ...
  describe('Geospatial Location', () => {
    ...
    it('should have 2dsphere index on location', () => {
      ...
      const locationIndex = indexes.find(
        (index) => index[0].location === '2dsphere',
      );
  ...
  describe('Indexes', () => {
    it('should have correct indexes defined', () => {
      ...
      const userDefaultIndex = indexes.find(
        (index) => index[0].userId === 1 && index[0].isDefault === 1,
      );
      expect(userDefaultIndex).toBeDefined();

      const pincodeCity = indexes.find(
        (index) => index[0].pincode === 1 && index[0].city === 1,
      );
  ...
  describe('Instance Methods', () => {
    ...
    it('should calculate distance from another point', () => {
      ...
      const distanceFrom = function (
        longitude: number,
        latitude: number,
      ): number {
        const R = 6371; // Earth's radius in kilometers
        const dLat =
          ((latitude - this.location.coordinates[1]) * Math.PI) / 180;
        const dLon =
          ((longitude - this.location.coordinates[0]) * Math.PI) / 180;
        const a =
      ...
      const distance = mockAddress.distanceFrom(72.8777, 19.076);
