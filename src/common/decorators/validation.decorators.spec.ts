...
import { IsValidUUID, IsValidPincode, IsValidLatitude, IsValidLongitude, IsPositiveNumber, IsValidAmount, IsValidQuantity } from './validation.decorators';
...
describe('Validation Decorators', () => {
  ...
  describe('IsValidPincode', () => {
    it('should pass for valid Indian pincode', async () => {
      ...
      const errors = await validate(testInstance);
      const pincodeErrors = errors.filter(error => error.property === 'pincode');
    ...
    it('should fail for invalid length', async () => {
      ...
      const pincodeErrors2 = errors2.filter(error => error.property === 'pincode');
  ...
  describe('IsValidLatitude', () => {
    ...
    it('should fail for latitude out of range', async () => {
      ...
      testInstance.latitude = -91;
      const errors2 = await validate(testInstance);
      const latErrors2 = errors2.filter(error => error.property === 'latitude');
  ...
  describe('IsValidLongitude', () => {
    it('should pass for valid longitude values', async () => {
      ...
      for (const lng of validLongitudes) {
        ...
        const errors = await validate(testInstance);
        const lngErrors = errors.filter(error => error.property === 'longitude');
    ...
    it('should fail for longitude out of range', async () => {
      ...
      testInstance.longitude = -181;
      const errors2 = await validate(testInstance);
      const lngErrors2 = errors2.filter(error => error.property === 'longitude');
  ...
  describe('IsPositiveNumber', () => {
    it('should pass for positive numbers', async () => {
      ...
      for (const num of positiveNumbers) {
        ...
        const errors = await validate(testInstance);
        const numErrors = errors.filter(error => error.property === 'positiveNumber');
    ...
    it('should fail for zero and negative numbers', async () => {
      ...
      const errors2 = await validate(testInstance);
      const numErrors2 = errors2.filter(error => error.property === 'positiveNumber');
  ...
  describe('IsValidQuantity', () => {
    ...
    it('should fail for zero and negative quantities', async () => {
      ...
      const errors2 = await validate(testInstance);
      const qtyErrors2 = errors2.filter(error => error.property === 'quantity');
